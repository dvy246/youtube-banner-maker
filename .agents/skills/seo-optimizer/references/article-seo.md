...
3-6 questions, phrased exactly as a real US searcher would ask them (typed or spoken), each with a direct-answer-first response. Mark up with `FAQPage` schema — see `references/schema-faq.md` for the exact JSON-LD template if this article's page can include structured data. Do not copy an FAQ block from another page on the same site — it must be specific to this article's actual content.

## Step 6 — Schema (if the article lives on a page you control)

At minimum, `Article` schema with a real `author`, `datePublished`, `dateModified`. Add `FAQPage` schema for the FAQ block (Step 5). See `references/schema-faq.md` for templates and validation guidance.
