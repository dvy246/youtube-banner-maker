...
the technical issues specific to SaaS marketing sites; for a full live-site technical audit, hand off
to **seo-mastery** (`references/technical-seo.md` and `references/pre-launch-indexing-audit.md`) or
**seo-geo-aeo**, which can actually fetch and inspect a real URL. This file exists to inform planning
decisions before or alongside that audit, not to replace it.
...
needs to be unambiguous. For a live-site check of current canonical setup, use seo-mastery's
pre-launch indexing audit rather than guessing from the codebase.
